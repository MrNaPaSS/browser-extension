...
3. Предоставьте доступ если все ОК"""
    ...
    async def confirm_deposit_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # Второе сообщение с кнопками: бот сигналов, академия, закрытый форум
            ...
            signal_keyboard = [
                ...
                [InlineKeyboardButton("🎓 Академия Здравого Трейдера", web_app=WebAppInfo(url="https://tradeacademy.onrender.com"))],
                [InlineKeyboardButton("💬 Торговый Форум", url="https://t.me/+hobQYO9vXZIxMTMy")]
            ]
...
class DataRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith('/verify'):
            try:
                # Parse UID from query: /verify?uid=1234567
                ...
                if not uid:
                    ...
                    self.end_headers()
                    self.wfile.write(b'{"error": "Missing uid parameter"}')
                    return
                ...
                # Normalize UID
                uid_clean = uid.strip()
                uid_with_po = f"PO{uid_clean}" if not uid_clean.startswith('PO') else uid_clean
                uid_without_po = uid_clean[2:] if uid_clean.startswith('PO') else uid_clean
                
                is_verified = False
                for u_id, u_data in users_db.items():
                    po_id = str(u_data.get('pocket_option_id', ''))
                    if po_id and (po_id == uid_with_po or po_id == uid_without_po or po_id == uid_clean):
                        # MUST BE BOTH VERIFIED AND DEPOSITED
                        if u_data.get('verified') and u_data.get('deposited'):
                            is_verified = True
                            break
                
                response = json.dumps({"verified": is_verified, "uid": uid_clean})
        ...
        if self.path.startswith('/users'):
            try:
                logger.info(f"📊 Academy API request: {self.path} from {self.client_address}")
                ...
                logger.info(f"✅ Academy data sent successfully for {self.path}")
                return
            except Exception as e:
        ...
        # Главная страница (для проверки, что бот в сети)
        if self.path == '/':
            self.send_response(200)
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write(b"Bot is online! Use /users or /verify")
            return
...
def start_ngrok_tunnel():
    """Автоматический запуск туннеля ngrok с постоянным доменом"""
    try:
        static_url = "cytoclastic-audria-overlightly.ngrok-free.dev"
        logger.info(f"🌐 Попытка запуска туннеля ngrok на {static_url}...")
        
        # Убиваем старые процессы ngrok, чтобы не было конфликтов
        if os.name == 'nt':
            subprocess.run(['taskkill', '/F', '/IM', 'ngrok.exe', '/T'], capture_output=True)
        
        # Запускаем ngrok в фоновом режиме
        # Используем .\ngrok если файл в текущей папке, или просто ngrok если в PATH
        cmd = ['ngrok', 'http', f'--url={static_url}', '8081']
        ...
        logger.info(f"✅ Туннель ngrok инициирован (PID: {process.pid})")
        logger.info(f"🔗 Ссылка: https://{static_url}")
        return process
    except Exception as e:
        logger.error(f"❌ Не удалось запустить ngrok автоматически: {e}")
        return None
...
def main():
    ...
    try:
        ...
        logger.critical(f"❌ КРИТИЧЕСКАЯ ОШИБКА: Не удалось запустить API сервер: {e}")
        return
